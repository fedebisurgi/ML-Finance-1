# -*- coding: utf-8 -*-
"""
Target3 SUPERADOR v3.0 — Full Rewrite
======================================
...
OUT_FILE_FULL = BASE_DIR / f"Pred_T3_v3_FULL_{DATE_STAMP}_{INPUT_HASH}.xlsx"
OUT_FILE_FILTER = BASE_DIR / f"Pred_T3_v3_MINPRICE_{str(MIN_PRICE_FILTER).replace('.','_')}_{DATE_STAMP}_{INPUT_HASH}.xlsx"
...
print("\n" + "=" * 80)
print("PIPELINE v3.0 — Starting dual execution")
print("=" * 80)
...
# FIX E2: the second run could reuse imputer/models from the first
# if the filter only removes rows (subset). For now, we retrain because
# the training distribution changes with the price filter.
# If you want speed and the filter is mild, set reuse_from_cache=True.
result_filter = run_pipeline(
    df_all=df,
...
print("\n" + "=" * 80)
print("PIPELINE v3.0 — COMPLETED")
print(f"File 1: {OUT_FILE_FULL}")
print(f"File 2: {OUT_FILE_FILTER}")
print(f"Input hash: {INPUT_HASH}")
print("=" * 80)